...
import { Plus, DotsThree, List, Link, Pencil, Trash, ArrowUp, ArrowDown } from "phosphor-react";
...
const ChatFolder: React.FC = () => {
  ...
  const AnimatedFolderIcon: React.FC = () => {
    ...
    return (
      ...
      >
        <div className={`transition-all duration-500 ${isAnimating ? 'animate-pulse' : ''}`}>
          {/* Main folder body */}
          <div className="w-24 h-20 bg-amber-200 rounded-lg relative overflow-hidden border border-amber-300">
            ...
            <div className={`absolute transition-all duration-700 ${isAnimating ? '-top-1 scale-110' : '-top-2'}`}>
              <div className={`absolute left-2 w-8 h-4 bg-orange-300 rounded-t-md transition-all duration-500 ${isAnimating ? 'bg-orange-400 translate-y-1' : ''}`}></div>
              <div className={`absolute left-12 w-8 h-4 bg-blue-400 rounded-t-md transition-all duration-700 ${isAnimating ? 'bg-blue-500 translate-y-1' : ''}`}></div>
              <div className={`absolute left-22 w-8 h-4 bg-red-400 rounded-t-md transition-all duration-900 ${isAnimating ? 'bg-red-500 translate-y-1' : ''}`}></div>
            </div>
            
            {/* Folder body animation */}
            <div className={`w-full h-full bg-amber-300 rounded-lg border border-amber-400 transition-all duration-500 ${isAnimating ? 'bg-amber-400 shadow-md' : ''}`}>
              {/* Inner files animation */}
              {isAnimating && (
                <div className="absolute inset-2 space-y-1">
                  <div className="h-1 bg-amber-600 rounded opacity-60 animate-pulse"></div>
                  <div className="h-1 bg-amber-600 rounded opacity-60 animate-pulse" style={{animationDelay: '0.1s'}}></div>
                  <div className="h-1 bg-amber-600 rounded opacity-60 animate-pulse" style={{animationDelay: '0.2s'}}></div>
                </div>
              )}
              ...
              {isAnimating && (
                <div className="absolute inset-x-2 top-6 space-y-2">
                  <div className="flex gap-1">
                    <div className="w-4 h-1 bg-blue-500 rounded animate-bounce" style={{animationDelay: '0.3s'}}></div>
                    <div className="w-4 h-1 bg-red-500 rounded animate-bounce" style={{animationDelay: '0.4s'}}></div>
                    <div className="w-4 h-1 bg-green-500 rounded animate-bounce" style={{animationDelay: '0.5s'}}></div>
                  </div>
                </div>
              )}
            </div>
  ...
  const handleDrop = (e: React.DragEvent, dropId: string) => {
    ...
    if (draggedItem && draggedItem !== dropId) {
      const draggedIndex = folders.findIndex(folder => folder.id === draggedItem);
  ...
  const ChatFolderItem: React.FC<ChatFolderItemProps> = ({ 
    ...
    <div 
      className={`group flex items-center justify-between py-3 px-2 rounded-md transition-colors cursor-pointer
        ...
        ${isDragOver && !isDragging ? 'bg-muted border-2 border-dashed border-primary' : ''}
      `}
      draggable={!isDefault}
